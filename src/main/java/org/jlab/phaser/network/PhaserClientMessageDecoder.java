...
public class PhaserClientMessageDecoder extends ChannelInboundHandlerAdapter {
  ...
  private static final Logger LOGGER = Logger.getLogger(PhaserClientMessageDecoder.class.getName());
  ...
  private void notifyJob(
    ...
    synchronized (notificationListeners) {
      for (NotificationListener listener : notificationListeners) {
        listener.jobNotification(
            job, jobId, jobStart, paused, loop, cavity, cavityStart, progress, label);
      }
    }
  }
      ...
      switch (notificationType) {
        ...
        case "job":
          ...
          if (json.containsKey("job") && !json.isNull("job")) {
            ...
            JsonNumber maxPhaseErrorNumber = jobJson.getJsonNumber("max-phase-error");
            float maxPhaseError = (float) maxPhaseErrorNumber.doubleValue();
            JsonNumber maxMomentumErrorNumber = jobJson.getJsonNumber("max-momentum-error");
            ...
            JsonArray cavitiesArray = jobJson.getJsonArray("cavities");
            ...
            job =
                new JobSpecification(
                    correct, continuous, maxPhaseError, maxMomentumError, kickSamples, cavities);

            if (json.containsKey("job-id") && !json.isNull("job-id")) {
              jobId = json.getJsonNumber("job-id").longValueExact();
            }
        ...
        case "progress":
          ...
          break;
        default:
          LOGGER.log(Level.SEVERE, "Unrecognized notification: {0}", notificationType);
          break;
      }
    } else {
    ...
    if (message instanceof JsonObject) {
      ...
      try {
        if (json.containsKey("response")) {
          ...
          LOGGER.log(Level.SEVERE, "Unrecognized message: {0}", message);
        ...
        notifyStatus("Server protocol not understood (unable to parse server message)", true);
      }
    } else {
