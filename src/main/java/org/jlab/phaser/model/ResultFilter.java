 ...
 *
 * Individual filter parameters are ignored if null. The parameters which are not null are combined
 * with SQL "and".
 *
 * @author ryans
 */
public final class ResultFilter {
     ...
     */
    public ResultFilter(Long jobId, Date minCavityStartDate, Date maxCavityStartDate,
            String cavityName, Float minPhaseError, PhaserOutcome outcome) {
        this.jobId = jobId;
        this.minCavityStartDate = minCavityStartDate == null ? null : new Date(
                minCavityStartDate.getTime());
        this.maxCavityStartDate = maxCavityStartDate == null ? null : new Date(
                maxCavityStartDate.getTime());
    ...
    public Date getMinCavityStartDate() {
        return minCavityStartDate == null ? null : new Date(
                minCavityStartDate.getTime());
    }
    ...
    public Date getMaxCavityStartDate() {
        return maxCavityStartDate == null ? null : new Date(
                maxCavityStartDate.getTime());
    }
    ...
    public String toHumanWhereClause(String timestampFormat) {
        ...
        if (getMinCavityStartDate() != null) {
            w = "Min Cavity Start Date is " + dateFormatter.format(
                    getMinCavityStartDate());
            whereList.add(w);
        }

        if (getMaxCavityStartDate() != null) {
            w = "Max Cavity Start Date is " + dateFormatter.format(
                    getMaxCavityStartDate());
            whereList.add(w);
        }
        ...
        if (getMinPhaseError() != null) {
            w = "Min Phase Error is " + decimalFormatter.format(
                    getMinPhaseError());
            whereList.add(w);
        }
    ...
    /**
     * Assigns prepared statement parameters for the SQL where clause returned from
     * toSqlWhereClause.
     *
    ...
    public void assignStatementParameters(PreparedStatement stmt) throws
        ...
        if (getMinCavityStartDate() != null) {
            stmt.setDate(parameterIndex++, new java.sql.Date(
                    getMinCavityStartDate().getTime()));
        }

        if (getMaxCavityStartDate() != null) {
            stmt.setDate(parameterIndex++, new java.sql.Date(
                    getMaxCavityStartDate().getTime()));
        }
