...
public final class JobFilter {
  ...
  public String toHumanWhereClause(String timestampFormat) {
    ...
    if (getMinJobStartDate() != null) {
      w = "Min Job Start Date is " + dateFormatter.format(getMinJobStartDate());
      whereList.add(w);
    }

    if (getMaxJobStartDate() != null) {
      w = "Max Job Start Date is " + dateFormatter.format(getMaxJobStartDate());
      whereList.add(w);
    }
  ...
  public void assignStatementParameters(PreparedStatement stmt) throws SQLException {
    ...
    if (getMinJobStartDate() != null) {
      stmt.setDate(parameterIndex++, new java.sql.Date(getMinJobStartDate().getTime()));
    }

    if (getMaxJobStartDate() != null) {
      stmt.setDate(parameterIndex++, new java.sql.Date(getMaxJobStartDate().getTime()));
    }
  }
}
