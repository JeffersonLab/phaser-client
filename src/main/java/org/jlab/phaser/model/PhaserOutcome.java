 ...
 *
 * <p>Note: the order declared matters because we use PhaserOutcome.ordinal() to match GUI select
 * box index. The order that they are declared does not match the alphabetical order in javadoc. The
...
public enum PhaserOutcome {
  ...
  SKIPPED,
  /**
   * Cavity was bypassed (skipped) because the control system has designated the cavity as bypassed.
   */
  BYPASSED,
  /**
  ...
  CORRECTED,
  /**
   * Server determined that the cavity could not be corrected and cannot be tried again (phase angle
   * error too great or measure date too old for example).
   */
