...
/**
 * Handles the "eLog" button click on the results form. Launches a new worker to
 * export results to the Jefferson Lab electronic logbook.
 *
 * @author ryans
 */
        ...
        new MinimumExecutionSwingWorker<Long, Void>() {
            ...
            protected Long doWithMinimumExecution() throws Exception {
                ...
                ResultPage page = dialog.getResultsCommandConsole().results(filter,
                        new Paginator(0, 0, Long.MAX_VALUE));
            ...
            protected void done() {
                try {
                    ...
                    JOptionPane.showMessageDialog(dialog, new HyperLinkEnabledMessage(html), "Successfully created eLog", JOptionPane.INFORMATION_MESSAGE);
                    ...
                    if (cause != null && (cause instanceof LogException || cause instanceof LogRuntimeException)) {
