...
/**
 * A comparator for programmatically sorting in the same way a JTable RowSorter
 * applied to a table with a CavityWithLastCorrectionTableModel performs.
 *
 * This is useful when moving CavityWithLastCorrection records from the
 * "available" table to the "scheduled" table in the new job dialog via the
 * batch add feature as we must move entities from zones/areas using whatever
 * sort order the user has currently set on the "available" table.
 *
 * CavityWithLastCorrection record sorting is done via cavity name
 * alphabetically (which happens to be s-coordinate order too) and via last
 * phase correction date. Which key is first and which is second and also the
 * direction of each key (ascending or descending) is configurable by the user.
 * The user interacts with the JTable by clicking column headers to toggle
 * primary key and direction. The batch dialog then must be prepared to honor
 * whatever sort order the user has selected in the view, and this comparator
 * takes care of that.
 *
 * @author ryans
 */
public class CavityWithLastCorrectionComparator implements Comparator<CavityWithLastCorrection> {
    ...
    /**
     * Create a new CavityWithLastCorrectionComparator given a list of Swing
     * sort keys.
     *
