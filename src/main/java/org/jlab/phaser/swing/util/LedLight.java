...
public class LedLight extends JPanel {
  ...
  private final Color transparentBackgroundColor =
      new Color(
          getBackground().getRed(), getBackground().getGreen(), getBackground().getBlue(), 128);
  ...
  public void paintComponent(Graphics g) {
    ...
    Shape circle =
        new Ellipse2D.Double(
            (getWidth() / 3) - 1, (getHeight() / 3) - 1, getWidth() / 3, getHeight() / 3);
    g2.setColor(socketColor);
    g2.draw(circle);

    Point2D center = new Point2D.Float((getWidth() / 2) - 1, (getHeight() / 2) - 1);
