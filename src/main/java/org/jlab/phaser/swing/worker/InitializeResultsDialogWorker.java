 ...
 * with initial values.
 *
 * First we query for the JobPage and if successful we next query for the ResultPage. We use the
 ...
 * @author ryans
 */
public final class InitializeResultsDialogWorker extends
        MinimumExecutionSwingWorker<JobPage, Void> {

    private static final Logger LOGGER = Logger.getLogger(
            InitializeResultsDialogWorker.class.getName());
    ...
    protected JobPage doWithMinimumExecution() throws Exception {
        return frame.getDatabaseConsole().jobs(dialog.getJobFilter(),
                dialog.getJobsPaginator());
    }
    ...
    protected void done() {
        ...
        try {
            ...
            /**
             * Use callback function to handle second worker finish because the existing "done"
             * method will try to hide the ResultDialog wait, but we need to hide the
             * PhaserClientFrame wait and also to show the ResultDialog. We could have overridden
             * ResultWorker and redefined the done method, but this callback will work and the
             * existing done method won't do any harm and sets the results for us.
             */
            secondWorker.addPropertyChangeListener(new PropertyChangeListener() {
