 ...
 *
 * This class delegates the heavy lifting to the OracleService, but provides
 * services of its own including serializing access to the database and wrapping
 * exceptions in CommandExceptions.
 *
 * To serialize access to the database all of the methods are synchronized. This
 * will prevent competing SwingWorker threads from executing database queries
 * concurrently.
 *
 * @author ryans
 */
public class OracleJdbcConsole implements DatabaseConsole {
    ...
    @Override
    public synchronized ResultPage results(ResultFilter filter, Paginator paginator) throws
            CommandException {
        try {
            long count = service.countResults(filter);
            List<ResultRecord> records = service.findResults(filter, paginator);
            return new ResultPage(records, new Paginator(count,
                    paginator.getOffset(),
                    paginator.getMaxPerPage()), filter);
    ...
    @Override
    public synchronized JobPage jobs(JobFilter filter, Paginator paginator) throws
            CommandException {
        try {
            long count = service.countJobs(filter);
            List<JobRecord> records = service.findJobs(filter, paginator);
            return new JobPage(records, new Paginator(count,
                    paginator.getOffset(),
                    paginator.getMaxPerPage()), filter);
        } catch (SQLException e) {
    ...
    @Override
    public synchronized LinkedHashSet<CavityWithLastCorrection> cavitiesWithLastCorrection() throws CommandException {
