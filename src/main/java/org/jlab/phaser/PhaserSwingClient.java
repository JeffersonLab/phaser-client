...
import org.jlab.phaser.swing.MaydayFrame;
 ...
 *
 * The PhaserSwingClient class loads the client properties, initializes a network connection to the
 * server, wires up callbacks, and initializes the Swing GUI.
 *
 * @author ryans
 */
public final class PhaserSwingClient {
     ...
     */
    public PhaserSwingClient(String host, int port) throws InterruptedException,
            InitializationException, ShutdownException {
        final PhaserClientMessageDecoder phaserDecoder
                = new PhaserClientMessageDecoder();
        NettyJsonConsole jobConsole = new NettyJsonConsole();
        OracleJdbcConsole dbConsole
                = new OracleJdbcConsole();
        PhaserClientFrame frame
                = new PhaserClientFrame(jobConsole, dbConsole);
        ...
        try {
            ...
            if (!future.isSuccess()) {
                throw new InitializationException("Unable to connect to server",
                        future.cause());
            }
            ...
            try {
                ...
                throw new InitializationException(
                        "Unable to fetch cavity list from server", e);
            }
            ...
            if (!userRequestedExit) {
                LOGGER.log(Level.SEVERE,
                        "Connection to server was closed without user consent");
            ...
            if (!future.isSuccess()) {
                throw new ShutdownException(
                        "Unable to gracefully close connection to server",
                        future.cause());
            }
        } finally {
    ...
    public static void main(String[] args) {
        try (InputStream propStream
            ...
            LOGGER.log(Level.SEVERE, "Unable to load client properties",
                    e);
            MaydayFrame mayday = new MaydayFrame(
